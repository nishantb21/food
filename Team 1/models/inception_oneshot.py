from .base_models import inceptionv1
from keras import Model, Input
from keras import backend as K
from keras.layers import merge, Dense, Flatten


class inception_oneshot():
    def __init__(self, dimensions):
        img_shape = tuple(dimensions)
        left_input = Input(img_shape, name='left_input')
        right_input = Input(img_shape, name='right_input')

        model = inceptionv1(dimensions).get_model()
        model_augmented = Model(inputs=[model.input], outputs=[model.get_layer('flattened_main').output])
...
if __name__ == '__main__':
    obj = inception_oneshot([224, 224, 3])
